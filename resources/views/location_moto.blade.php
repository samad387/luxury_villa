...
<section style="position: relative; width: 100%; height: 450px; overflow: hidden;">
    <img src="https://images.unsplash.com/photo-1558980664-769d59546b3d?auto=format&fit=crop&w=1200&q=80"
         alt="Moto de Prestige"
    ...
    <div class="etablissement" onclick="window.location.href='{{ route('public.transport.show', $moto) }}'">
        <div class="etablissement-image">
            @if($moto->images && $moto->images->isNotEmpty())
                <img src="{{ asset('storage/' . $moto->images->first()->path) }}" alt="{{ $moto->nom }}">
            @elseif($moto->image)
                <img src="{{ asset('storage/' . $moto->image) }}" alt="{{ $moto->nom }}">
            @else
                <img src="https://via.placeholder.com/400x200?text=Moto" alt="{{ $moto->nom }}">
            @endif
        </div>
...
@empty
    <p style="grid-column: 1 / -1; text-align: center; padding: 40px; color: #666;">Aucune moto disponible pour le moment.</p>
@endforelse
</div>
@endsection
