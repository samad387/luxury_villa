...
@section('title', 'Riad de Luxe - Marrakech')
...
<div class="container">
  <main class="main-content">
    <h1 class="riad-title">{{$riad->name}}</h1>
    @if ($riad->images->isNotEmpty())
    <div class="main-image-container" onclick="openFullscreen()">
      <img id="mainImage" src="{{asset('storage/' . ($riad->images->first()->path ?? ''))}}">
    ...
    <img src="{{asset('storage/' . $image->path)}}" class="{{ $image->id == $riad->images->first()->id ? 'active' : ''}}" onclick="changeImage(this)">
    @endforeach
    </div>
    @endif
    ...
    <section class="location">
     ...
     <section class="location-section">
                ...
                @if ($riad->geo_emplacement && $riad->coordinates)
                    <p>{{ $riad->geo_emplacement }}</p>
                    <div id="mapid"></div> {{-- Removed Tailwind classes --}}
                @else
                    <p class="no-map-text">Aucune localisation géographique fournie pour cette riad.</p> {{-- Applied custom class --}}
                @endif
            </section>
    </section>
  </main>
  ...
  @if(session('success'))
    ...
    @endif
    <h3>Réserver cette Riad</h3>
    <form action="{{ route('reservations.store') }}" method="POST">
      @csrf
      <input type="hidden" name="car_model" value="{{$riad->name}}">
