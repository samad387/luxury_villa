...
@section('title', 'Edit riad: ' . $riad->name)

@section('content')
    <div class="container mx-auto">
        <h2 class="text-3xl font-bold text-gray-800 mb-6">Edit riad: <span class="text-indigo-600">{{ $riad->name }}</span></h2>

        <div class="bg-white rounded-xl shadow-lg p-8">
            <form action="{{ route('admin.riads.update', $riad->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')

                <!-- Name Field -->
                <div class="mb-5">
                    <label for="name" class="block text-gray-700 text-sm font-bold mb-2">riad Name:</label>
                    <input type="text" name="name" id="name" class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200 @error('name') border-red-500 @enderror" value="{{ old('name', $riad->name) }}" required>
                    @error('name')
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                        @foreach ($riad->images as $image)
                            <div class="relative group rounded-lg overflow-hidden shadow-md">
                                <img src="{{ asset('storage/' . $image->path) }}" alt="riad Image" class="w-full h-32 object-cover transition-transform duration-200 transform group-hover:scale-105">
                ...
                <div class="flex items-center justify-end">
                    <button type="submit" class="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white text-lg font-semibold rounded-lg shadow-md transition duration-200 ease-in-out transform hover:scale-105">
                        <i class="fas fa-save mr-2"></i> Update riad
                    </button>
                    <a href="{{ route('admin.riads.index') }}" class="ml-4 px-6 py-3 bg-gray-300 hover:bg-gray-400 text-gray-800 text-lg font-semibold rounded-lg shadow-md transition duration-200 ease-in-out">
                        Cancel
                    </a>
                </div>
            </form>
