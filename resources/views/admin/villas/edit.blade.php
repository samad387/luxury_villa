...
@section('content')
    <div class="container mx-auto">
        <h2 class="text-3xl font-bold text-gray-800 mb-6">Edit Villa: <span class="text-indigo-600">{{ $villa->name }}</span></h2>
        ...
        <div class="bg-white rounded-xl shadow-lg p-8">
            <form action="{{ route('admin.villas.update', $villa->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="name" id="name" class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200 @error('name') border-red-500 @enderror" value="{{ old('name', $villa->name) }}" required>
                    @error('name')
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                        @foreach ($villa->images as $image)
                            <div class="relative group rounded-lg overflow-hidden shadow-md">
                                <img src="{{ asset('storage/' . $image->path) }}" alt="Villa Image" class="w-full h-32 object-cover transition-transform duration-200 transform group-hover:scale-105">
                ...
                <div class="flex items-center justify-end">
                    ...
                    <a href="{{ route('admin.villas.index') }}" class="ml-4 px-6 py-3 bg-gray-300 hover:bg-gray-400 text-gray-800 text-lg font-semibold rounded-lg shadow-md transition duration-200 ease-in-out">
                        Cancel
                    </a>
                </div>
            </form>
