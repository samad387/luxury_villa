...
@section('title', 'Modifier Jet')

@section('content')
<div class="bg-white rounded-xl shadow p-6 max-w-3xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">✏️ Modifier Jet</h2>
        <a href="{{ route('admin.jets.index') }}" class="text-gray-600 hover:text-gray-800">
            ← Retour à la liste
        </a>
    </div>
    ...
    <form method="POST"
          ...
          action="{{ route('admin.jets.update', $jet) }}"
          class="space-y-6">
        @csrf
        @method('PUT')

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Nom du jet *</label>
            <input type="text" 
                   name="name" 
                   value="{{ old('name', $jet->name) }}"
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" 
                       name="capacity" 
                       value="{{ old('capacity', $jet->capacity) }}"
                       min="1"
                       class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                       required>
            </div>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Autonomie (km) *</label>
                <input type="number" 
                       name="range_km" 
                       value="{{ old('range_km', $jet->range_km) }}"
                       step="0.01"
                       min="0"
                       class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                       required>
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Prix par heure (€) *</label>
            <input type="number" 
                   name="price_per_hour" 
                   value="{{ old('price_per_hour', $jet->price_per_hour) }}"
                   step="0.01"
                   min="0"
                   class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                   required>
        ...
        <div>
            ...
            <textarea name="description" 
                      rows="5"
                      class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      required>{{ old('description', $jet->description) }}</textarea>
        </div>

        <!-- Existing Images -->
        @if($jet->images->isNotEmpty() || $jet->image)
        <div class="mb-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                @foreach ($jet->images as $image)
                    <div class="relative group rounded-lg overflow-hidden shadow-md">
                        <img src="{{ asset('storage/' . $image->path) }}" alt="Jet Image" class="w-full h-32 object-cover transition-transform duration-200 transform group-hover:scale-105">
                ...
                @endforeach
                @if($jet->image && $jet->images->isEmpty())
                    <div class="relative group rounded-lg overflow-hidden shadow-md">
                        <img src="{{ asset('storage/' . $jet->image) }}" alt="Jet Image" class="w-full h-32 object-cover transition-transform duration-200 transform group-hover:scale-105">
                        <div class="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
        ...
        <div class="flex items-center">
            ...
            <input type="checkbox" 
                   ...
                   value="1" 
                   id="active"
                   {{ old('active', $jet->active) ? 'checked' : '' }}
                   class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
            <label for="active" class="ml-2 block text-sm text-gray-700">
                Jet actif (affiché sur le site public)
            </label>
        </div>
        ...
        <div class="flex gap-4">
            <button type="submit" class="bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 transition font-medium text-lg">
                <i class="fas fa-save mr-2"></i> Update Jet
            </button>
            <a href="{{ route('admin.jets.index') }}" class="bg-gray-200 text-gray-700 px-6 py-3 rounded-lg hover:bg-gray-300 transition font-medium">
                Annuler
            </a>
        </div>
    </form>
